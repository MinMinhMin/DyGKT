...
class DecayingCumulativeMomentumEncoder(nn.Module):
    """
    Computes per-step decaying cumulative momentum from interaction timestamps.

    Intuition (educational context):
      - Small ΔT between consecutive interactions → high intensity (student is "in the zone")
      - Momentum accumulates over the subwindow with learnable exponential decay α
      - No hard threshold: α is learned end-to-end via sigmoid

    Math:
      intensity_i = 1 / (ΔT_i + ε),   ΔT_i = t_i - t_{i-1}
      momentum_i  = α · momentum_{i-1} + (1 - α) · intensity_i     [EMA scan]

    Args:
        node_dim : output feature dimension (same as node_dim in DyGKT)
        epsilon  : numerical stability floor for 1/ΔT (in the same unit as timestamps)
    """

    def __init__(self, node_dim: int, epsilon: float = 1.0):
        super().__init__()
        self.epsilon = epsilon

        # α = sigmoid(alpha_raw) ∈ (0, 1)  — how fast old momentum decays
        self.alpha_raw = nn.Parameter(torch.zeros(1))

        # scalar momentum → node_dim feature vector
        self.proj = nn.Linear(1, node_dim, bias=True)
        self.norm = nn.LayerNorm(node_dim)

    @property
    def alpha(self) -> torch.Tensor:
        return torch.sigmoid(self.alpha_raw)          # scalar ∈ (0, 1)

    def forward(self, timestamps: torch.Tensor) -> torch.Tensor:
        """
        Args:
            timestamps : (B, L) — absolute timestamps, oldest → newest
        Returns:
        ...
        # ── 1. Inter-event delta ─────────────────────────────────────────
        # delta[i] = t[i] - t[i-1]
        # For i=0, pad by replicating delta[1] (avoids zero-division at the boundary)
        delta_t = timestamps[:, 1:] - timestamps[:, :-1]          # (B, L-1)
        delta_t = torch.cat([delta_t[:, :1], delta_t], dim=1)     # (B, L)
        delta_t = delta_t.abs()                                    # guard negatives

        # ── 2. Interaction intensity ─────────────────────────────────────
        # Compress range with log1p so extreme ΔT values don't dominate
        intensity = torch.log1p(1.0 / (delta_t + self.epsilon))   # (B, L)

        # ── 3. EMA scan (decaying cumulative momentum) ───────────────────
        alpha = self.alpha                                         # scalar tensor
        steps = [intensity[:, 0]]                                  # m_0 = intensity_0
        for i in range(1, L):
            steps.append(alpha * steps[-1] + (1.0 - alpha) * intensity[:, i])
        momentum = torch.stack(steps, dim=1)                       # (B, L)

        # ── 4. Project scalar → node_dim ─────────────────────────────────
        feat = self.proj(momentum.unsqueeze(-1))                   # (B, L, node_dim)
...
class TimeDualDecayEncoder(nn.Module):
    ...
    def forward(self, timestamps: torch.Tensor):
        ...
        # Tensor, shape (batch_size, seq_len, time_dim)
        # print("timestamps_short.sha",timestamps_short.shape)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, inputs_query: torch.Tensor, inputs_key: torch.Tensor = None, inputs_value: torch.Tensor = None,
        ...
        if neighbor_masks is not None:
            ...
            # attn_mask = torch.zeros((neighbor_masks.shape[1],neighbor_masks.shape[1]),device=neighbor_masks.device)
        ...
        hidden_states = self.multi_head_attention(query=transposed_inputs_query, key=transposed_inputs_key,
                                                  value=transposed_inputs_value, key_padding_mask=neighbor_masks, attn_mask=attn_mask)[0].transpose(0, 1)
